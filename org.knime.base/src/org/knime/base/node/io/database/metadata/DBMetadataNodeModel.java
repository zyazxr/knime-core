 ...
 * @author adewi
 */
class DBMetadataNodeModel extends NodeModel implements FlowVariableProvider {

    private DatabaseConnectionSettings m_settings = new DatabaseConnectionSettings();

    private SettingsModelDBMetadata m_metadataSettings = DBMetadataDialogPane.getMetadataModel();
...
//        m_querySettings.saveConnection(settings);

//        m_settings.saveConnection(settings);

        m_metadataSettings.saveSettingsTo(settings);
    }
...
//
//        DatabaseConnectionSettings s2 = new DatabaseConnectionSettings();
//        s2.validateConnection(settings, getCredentialsProvider());

        m_metadataSettings.validateSettings(settings);
    }
...
//        m_querySettings.loadValidatedConnection(settings, getCredentialsProvider());

//        m_settings.loadValidatedConnection(settings, getCredentialsProvider());

        m_metadataSettings.loadSettingsFrom(settings);
    }
