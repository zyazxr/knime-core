...
/**
 * Default search filter for all DB objects.
 *
...
public class DefaultSearchFilter implements SearchFilter {
    ...
    /**
     * Constructor for the default search filter.
     *
     * @param regex the regular expression that should be matched
     */
    public DefaultSearchFilter(final Pattern regex) {
        m_regex = regex;
    }
    ...
    public boolean filter(final DBObject object) {
        if (object instanceof DBColumnContainer) {
            DBColumnContainer container = (DBColumnContainer)object;
            if (!m_regex.matcher(container.getName()).matches()
                && !m_regex.matcher(container.getSchemaName()).matches()) {
                return false;
            }
        }
