 ...
 * @since 3.3
 */
public class DBMetadataDialogPane extends NodeDialogPane {
    ...
    private final JCheckBox m_configureBox = new JCheckBox("Run SQL query only during execute, skips configure");

    private static final NodeLogger LOGGER = NodeLogger.getLogger(DBReaderDialogPane.class);
     ...
     */
    public DBMetadataDialogPane(final boolean showConnectionPanel, final boolean showDBBrowser) {
        // init SQL statement component
        m_statmnt.setPreferredSize(new Dimension(350, 200));
        m_statmnt.setFont(DBDialogPane.FONT);
        m_statmnt.setText("SELECT * FROM " + DatabaseQueryConnectionSettings.TABLE_PLACEHOLDER);

        // init database table browser
        final JPanel browserPanel = new JPanel(new BorderLayout());
        if (showDBBrowser) {
            browserPanel.setBorder(BorderFactory.createTitledBorder(" Database Browser "));
            JTextField filter = new JTextField();
            browserPanel.add(filter, BorderLayout.NORTH);
            final JButton updateButton = new JButton("Fetch Metadata");
            browserPanel.add(updateButton, BorderLayout.CENTER);
            final DBMetadataTreeBrowser browser = new DBMetadataTreeBrowser(m_statmnt);
            browserPanel.add(browser, BorderLayout.SOUTH);
            ...
            updateButton.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(final ActionEvent ae) {
                    ...
                    m_text = filter.getText();
                    try {
                        final DatabaseConnectionSettings settings = getConnectionSettings();
                        final DBReader conn =
                            settings.getUtility().getReader(new DatabaseQueryConnectionSettings(settings, ""));
                        final DBMetadata con = settings.getUtility().getDatabaseMetadata(settings.createConnection(getCredentialsProvider()));
                        browser.update((DatabaseMetaData)null, con);
                        ...
                        m_worker = new SwingWorkerWithContext<Void, Void>() {
                            ...
                            protected Void doInBackgroundWithContext() throws Exception {
                                try {
                                    final DatabaseMetaData meta = conn.getDatabaseMetaData(getCredentialsProvider());
                                    browser.update(meta, con);
                                } catch (Exception e) {
                                    LOGGER.warn("2Error during fetching " + "metadata from database, reason: "
                                        + e.getMessage());
                                }
                                return null;
                            }
                        ...
                        browser.update((DatabaseMetaData)null, (DBMetadata)null);
                        updateButton.setText("Fetch Metadata");
                        updateButton.setEnabled(true);
                        LOGGER.warn("1Error during fetching metadata " + "from database, reason: " + e.getMessage(), e);
                    }
                }
            });
        }
        ...
        super.addTab("Settings", browserPanel, false);
    }

    /**
     * @return false (default), or true if the option to run the SQL query without configure should be visible.
     */
    ...
    protected void loadSettingsFrom(final NodeSettingsRO settings, final PortObjectSpec[] specs)
        ...
        m_statmnt.setText(
            statement == null ? "SELECT * FROM " + DatabaseQueryConnectionSettings.TABLE_PLACEHOLDER : statement);
