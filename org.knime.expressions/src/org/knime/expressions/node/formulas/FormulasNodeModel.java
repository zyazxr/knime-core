...
import org.knime.expressions.node.SingleExpressionCellFactory;
...
public class FormulasNodeModel extends NodeModel {
	...
	protected DataTableSpec[] configure(final DataTableSpec[] inSpecs) throws InvalidSettingsException {
		if (m_configuration == null || m_configuration.getExpressionTable() == null
				|| m_configuration.getExpressionTable().length == 0) {
			return inSpecs;
		}
	...
	private ColumnRearranger createColumnRearranger(DataTableSpec inSpec, ExecutionContext exec) {
		...
		for (int i = 0; i < expressions[0].length; i++) {
			if (columnIndexMap.containsKey(expressions[0][i])) {
				...
				rearranger.insertAt(colIndex,
						new MultiExpressionCellFactory(new DataColumnSpec[] { replaceGenerator.newColumn(expressions[0][i], types[i]) }, columns, new String[] { expressions[1][i] }, columnIndexMap,
								new DataType[] { types[i] }, getAvailableFlowVariables(), exec));
			} else {
		...
		if (!expressionList.isEmpty()) {
			...
			rearranger.append(new MultiExpressionCellFactory(specArray, columns, expressionArray, columnIndexMap, typeArray,
